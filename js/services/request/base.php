...
abstract Class Base {
    ...
    public function __construct() {
        ...
        if (mysqli_connect_error()) {
            if (PRODUCCION) {
                throw new Exception('Error No. C001');
            } else {
                throw new Exception('Error: ' . mysqli_connect_errno() . '<br />Mensaje: ' . mysqli_connect_error());
            }
            return;
        }
        if (!$this->conexion->set_charset("utf8")) {
            throw new Exception("Error al querer cargar conjunto de caracteres UTF8.<br />Conjunto actual:<br />" . $this->conexion->character_set_name());
        }
    }
        ...
        if ($this->_elDato != '') {
            ...
            switch ($this->_elTipo) {
                ...
                case "Entero":
                    ...
                    break;
                case "Encriptalo":
                    $this->_elDato = "'" . hash('sha256', trim($this->_elDato)) . "'";
                    break;
                case "Fecha":
    ...
    protected function hora($laHora, $sinSegundos = '') {
        ...
        if ($sinSegundos == 'S') {
            if ((count($_laHora) == 3) and ( $_laHora[0] < 24 and $_laHora[0] >= 0) and ( $_laHora[1] < 60 and $_laHora[1] >= 0)) {
                return $laHora . ':00';
            } else {
                return '00:00:00';
            }
        } else {
            if ((count($_laHora) == 3) and ( $_laHora[0] < 24 and $_laHora[0] >= 0) and ( $_laHora[1] < 60 and $_laHora[1] >= 0) and ( $_laHora[2] < 60 and $_laHora[2] >= 0)) {
                return $laHora;
            } else {
                return '00:00:00';
            }
    ...
    protected function fecha($laFecha) {
        ...
        if ((count($_laFecha) == 3) and ( checkdate($_laFecha[1], $_laFecha[2], $_laFecha[0]))) {
            return $laFecha;
        } else {
            return '0000-00-00';
        }
        ...
        if ($_resultado) {
            ...
            switch ($tipoDevolucion) {
                ...
                case 8:
                    ...
                    break;
                case 9:
                    // Si utiliza procedimientos almacenados selecione esta opcion para devolver
                    // en id del ultimo registro realizado.
                    if ($_resultado) {
                        $_status = mysql_query("select @id as id;");
                        $_row = mysql_fetch_assoc($_status);
                        $_devolver = $_row['id'];
                    } else {
                        $_devolver = 0;
                    }
                ...
                case 10:
                    // Si utiliza procedimientos almacenados selecione esta opcion para devolver
                    // un 1 en caso de ejecutarse correctamente el procedimiento.
                    // un 0 en caso de no ejecutarse correctamente el procedimiento.
                    if ($_resultado) {
                        $_devolver = 1;
                    } else {
                        $_devolver = 0;
            ...
            if (PRODUCCION) {
                throw new Exception("SQL " . rand(10, 99) . $this->conexion->errno);
            } else {
    ...
    /**
     * Funcion que sirve para ver si el Empleado tiene Sesion o no.
     * @access protected
     * @return boolean True, si tiene Sesion, False si NO tiene Sesion.
     */
    protected function validaSesionUsuario() {
